# Internal Docs (pages/ submodule) Operations Rule

`pages/` is a submodule that serves a static site. It is the internal-docs store for HTML explainers, review results, incident reports, etc. linked from Issues / PRs / Discussions, and holds the single-page HTML that the `writing-html-explainer` skill generates.
...
pages:
  ...
  repo: <org>/<pages-repo>              # internal docs repo
  baseUrl: https://example.com          # base of the delivery URL (for link generation)
  defaultBranch: main
...
1. Prepare an internal-docs repo. For GitHub Pages, **public** is recommended. The **site assumes root-domain serving** (each page uses absolute paths like `/assets/...`), so prefer a custom domain (CNAME) or an org-pages repo (`<org>.github.io`). Avoid project-pages subpath serving (`<org>.github.io/<repo>/`) because absolute paths break there
...
7. Update `pages.baseUrl` to the finalized delivery URL
...
# 3. Push to main (← required; this is what delivers the change)
...
> **Required**: step 3 (the submodule push to `main`). Whatever the delivery method, nothing is delivered until you push.
...
## File Naming and Structure Selection
...
### Per-category structure choice

| Category | Approach | Reason |
|----------|----------|--------|
| `prs/` | Master + subpages (`index.html` + `*.html`) | Aggregate the entire PR lifecycle under one directory; `summary.html` / `review-r{n}.html` / `fix-r{n}.html` are linked from the master |
| `issues/`, `discussions/` | Directory (`{number}/index.html`) | Single-page-per-number is the default |
| `specs/`, `explainers/`, `reviews/`, `incidents/`, `status/` | Directory (`{topic}/index.html`) | Topic-slug-based, self-contained |

**Semantics**: A directory (`{topic}/index.html`) is a "single page aggregation unit"; a subpage (`{topic}/{name}.html`) is a "subpage under the same aggregation". The two coexist, and `index.html` always acts as the master (aggregation point). The index builder only indexes the master — subpages are reached via the master, and orphan subpages (no master in the directory) are reported as warnings.

## Notes

- It is an **internal-docs site**, but still do not include secrets (anyone with repo read access can see it; verify that any code snippets / screenshots are OK to publish)
- Never change a delivered URL (avoids broken links from Issues / PRs)
