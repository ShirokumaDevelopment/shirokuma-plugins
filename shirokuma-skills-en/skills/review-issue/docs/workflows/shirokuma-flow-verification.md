# shirokuma-flow Verification Workflow

## Overview

shirokuma-flow を使用したコード品質検証のワークフロー。
...
shirokuma-flow lint tests -p . -c .shirokuma/config.yaml -f terminal

# 2. Implementation-test coverage
shirokuma-flow lint coverage -p . -c .shirokuma/config.yaml

# 3. Documentation structure (if OVERVIEW.md/ADR exists)
shirokuma-flow lint docs -p . -c .shirokuma/config.yaml

# 4. Code structure check (Server Actions) - NEW
shirokuma-flow lint code -p . -c .shirokuma/config.yaml -f terminal
```
...
shirokuma-flow lint tests -p . -c .shirokuma/config.yaml -f summary
shirokuma-flow lint coverage -p . -c .shirokuma/config.yaml -f summary

# Strict mode for CI (exits with error code if issues remain)
shirokuma-flow lint tests -p . --strict
shirokuma-flow lint coverage -p . --strict -s
```
...
shirokuma-flow lint tests -p . -f summary
...
shirokuma-flow lint tests -p . --strict
shirokuma-flow lint coverage -p . -s

# Generate documentation
shirokuma-flow generate -p .
```
...
- **Testing Role**: `shirokuma-flow lint tests` + `lint coverage`
- **Code Role**: `shirokuma-flow lint code` (Server Action structure)
