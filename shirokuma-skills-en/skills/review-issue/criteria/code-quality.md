...
## Helper Extension Pattern: Backwards-Compatible Default Arguments

### Required Practices

When extending an existing helper to support new behavior, prefer **backwards-compatible default arguments** over duplicating the helper or rewriting every existing call site. Use a Union-typed default argument to switch behavior.

- [ ] **Use a Union type to constrain the default**: Restrict the value domain (e.g. `"A" | "B"`) and pin the default explicitly
- [ ] **Existing call sites remain identical**: Verify in code and tests that the default produces the original behavior
- [ ] **Document call-site routing in JSDoc**: Explain which call sites should pass a non-default value, with examples
- [ ] **Tests**: Cover the default path and the non-default path in separate cases

### Example

```typescript
// Bad: Duplicate as a separate function (code duplication, caller confusion)
...
// Good: Union-typed default argument switches behavior
/**
 * Close an Issue by ID.
 *
 * @param id - Issue GraphQL ID
 * @param stateReason - "COMPLETED" for normal completion (default), "NOT_PLANNED" for cancellation paths
 *
 * @example
 * ```typescript
 * await closeIssueById(id)                  // existing callers (COMPLETED)
 * await closeIssueById(id, "NOT_PLANNED")   // cancellation path
 * ```
 */
...
- Pre/post-extension behavior is identical for existing callers (does the default match the original contract?)
- The Union type statically constrains the value domain (avoid raw `string`)
- JSDoc concretely states *when* a non-default value should be passed
- Tests cover the default path (existing contract) and the non-default path (new behavior) in separate cases
