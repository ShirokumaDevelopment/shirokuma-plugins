---
name: publishing
description: Manages public releases via shirokuma-flow repo pairs CLI. Handles status checks, dry-run previews, release execution, and .shirokumaignore configuration. Triggers: "publish", "release", "public release", "repo pairs".
allowed-tools: Bash, Read, Grep, Glob, AskUserQuestion, TaskCreate, TaskUpdate, TaskGet, TaskList
---

# Publishing

Manages the release workflow from private development repositories to public repositories using `shirokuma-flow repo pairs` CLI.
...
- `shirokuma-flow` CLI installed and available in PATH
- `gh` CLI authenticated (`gh auth status`)
- `.shirokuma/config.yaml` exists in project root
- Repo pair configured (`shirokuma-flow repo pairs list`)
...
shirokuma-flow repo pairs list

# Check release status (latest tags on both repos)
shirokuma-flow repo pairs status <alias>

# Preview release (no changes made)
shirokuma-flow repo pairs release <alias> --tag <version> --dry-run

# Execute release
shirokuma-flow repo pairs release <alias> --tag <version>

# Verbose output for debugging
shirokuma-flow repo pairs release <alias> --tag <version> -v
```

### Initialize a New Repo Pair

```bash
shirokuma-flow repo pairs init <alias> \
  --private <owner/repo> \
  --public <owner/repo> \
  --exclude ".claude/" --exclude "docs/internal/"
```

Configuration is stored in `.shirokuma/config.yaml`.
...
Use `AskUserQuestion` to confirm the version number with the user.
...
```bash
shirokuma-flow repo pairs list  # Shows configured exclude patterns
```
