...
│   └── coding-claude-config/      # This skill
│
...
Files in `.claude/rules/shirokuma/` are auto-deployed by `shirokuma-flow init` (or `update-skills --with-rules`).
...
| **Generic patterns** | Skills `patterns/`, `criteria/` | Reusable, publishable |
...
**Important**: `.claude/rules/shirokuma/` is managed by the shirokuma-skills plugin (deployed via `shirokuma-flow init`). Files there are overwritten on update, so custom rules placed here will be lost. Place custom rules in `.claude/rules/` directly or in your own subdirectories.
...
1. **Read template**:
   ```bash
   cat .claude/skills/coding-claude-config/templates/rule.md.template
   ```
...
Rules are always loaded into the main context, but **do not have automatic access to skill `reference/` files**. File path references are not auto-expanded when rules are loaded, resulting in dead references.

### Prohibited Patterns

| Pattern | Problem | Fix |
|---------|---------|-----|
| `See coding-nextjs/reference/patterns.md` | File not expanded when rule loads | Include necessary details in the rule body |
| `Follow procedures in branch-workflow-details.md` | Path unclear, not expanded | Include necessary procedures in the rule body |
| `the above rule` | Ambiguous directive | `See branch-workflow rule` |

### Correct Reference Patterns

- **Rule → Rule**: Reference by rule name (e.g., `See branch-workflow rule`)
- **Rule → Skill details**: Include necessary details in the rule body. Rule context cannot access skill references — mention only the skill name (e.g., "this is managed by implement-flow skill")
- **Rule → External info**: Reference by Discussion number or document path
