...
| Data retrieval | Issue body embedded in Task agent | Plan issue identified from Issue number, fetched via `shirokuma-flow issue context` |
| Purpose | Initial quality gate for plan | Independent second opinion |
| Invocation | Auto-executed at `plan-issue` step 4 | `/analyze-issue plan #N` or Review Issue |
...
2. Fetch Issue body: shirokuma-flow issue context {number} (→ Read .shirokuma/github/{org}/{repo}/issues/{number}/body.md)
