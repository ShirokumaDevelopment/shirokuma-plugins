...
```
skill-name/
├── patterns/         # Reusable patterns (shareable)
├── templates/        # Code templates (shareable)
...
| `.shirokuma/config.*` exists | `rules/shirokuma-flow/` | `shirokuma-annotations.md` |
...
Language policy is handled by plugin-specific rules:

| Concern | Where |
|---------|-------|
| GitHub output language | `output-language` rule (English for EN plugin) |
| Config file language | `skill-authoring` rule (English) |
| Code/comment language | `git-commit-style` rule (English) |
...
- Common configs belong in `patterns/` or `templates/` directories
