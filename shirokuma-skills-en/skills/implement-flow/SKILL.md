...
| 6 | Present next-flow guidance (`/review-flow`) (**do NOT transition the issue or PR to Review. PR stays in Backlog**) | Presenting next-flow guidance | Manager direct |

Dependencies: step 2 blockedBy 1, step 3 blockedBy 2, step 4 blockedBy 3, step 5 blockedBy 4, step 6 blockedBy 5.

> **PR initial Status (#2802)**: The `pr create` invocation in step 3 creates the PR itself in **Status: Backlog**.

> **implement-flow does NOT auto-launch code review (#2818)**: The implementation chain does not auto-launch the AI review (`review-flow`) after PR creation. Code review is an "entry point for a new conversation" — it is work that should be launched through a user control point (UCP). At the end of the chain (step 6), **only present** `/review-flow #{PR#}` as guidance and leave the launch to the user.
>
> **The PR stays in Backlog through the end of the chain**: The `Backlog → Review` transition is performed by `review-flow` when the user runs `/review-flow` and the AI review PASSes. The implement-flow chain does NOT transition the PR to Review. It also does NOT transition the task issue or plan issue to Review (one-Review-per-entity principle).
>
> **finalize-changes is retained (step 4)**: Post-processing (`/simplify` + `reviewing-security` + `lint docs` + improvement commit) is post-processing distinct from code review and is outside user control, so it runs automatically after PR creation.

> **No-changes branch**: When `coding-worker` completes with `changes_made: false`, skip steps 2–4 (commit, PR, finalize-changes) and proceed to step 5 (no-changes work summary) → step 6 (next-flow guidance / status determination). See the "No-Changes Path" sections in [reference/chain-execution.md](reference/chain-execution.md) and [reference/chain-end-steps.md](reference/chain-end-steps.md) for details.
...
| Work Type | Chain |
|-----------|-------|
| General Coding | Work → Commit → PR(Backlog) → finalize-changes → Work Summary + PR Page Generation → /review-flow guidance |
| Research | Research → Discussion |
| Review | Review → Report posted → Complete (no commit/PR chain) |

> **End of the coding chain (#2818)**: `PR(Backlog)` denotes that `pr create` creates the PR in Backlog. After PR creation, run `finalize-changes` (post-processing), and after Work Summary + PR Page Generation, **present `/review-flow #{PR#}` as guidance and end the chain**. implement-flow does NOT auto-launch code review — the user launches the review explicitly via `/review-flow`. The PR stays in Backlog through the end of the chain; the `Backlog → Review` transition is performed by `review-flow` after an AI review PASS when the user runs `/review-flow`.
