...
- **Design artifact destinations**: Follow the "Investigation / Design Artifacts" section (#2251) of `rules/output-destinations.md`. Ideal-state designs and architecture proposals default to **Discussion (Knowledge)** or ADR; structured reference artifacts (process flows, side-effect matrices, gap analysis) default to **Discussion (Reports)**. Avoid dumping these into Issue comments — they get buried chronologically and become hard to reuse
