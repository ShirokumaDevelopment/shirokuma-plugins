...
| タイプ | 目的 | 出力先 | 寿命 |
|--------|------|--------|------|
| **作業レポート** | 作業中の人間レビュー | GitHub Discussions (Reports) | 一時的 |
| **最終ドキュメント** | プロジェクト全体の記録 | shirokuma-flow generate portal | 永続的 |

## 作業レポート

**用途**: レビューレポート、実装進捗、lint 結果

**出力先**: GitHub Discussions → Reports カテゴリ

```bash
# shirokuma-flow CLI 経由で作成（frontmatter にメタデータを含む）
...
- 人間の確認・フィードバック用
- 定期的にクリーンアップ可能
- GitHub のブラウザ UI で閲覧可能

## 最終ドキュメント

**用途**: 完成した機能ドキュメント、API リファレンス、アーキテクチャ図

**出力先**: shirokuma-flow generate portal
...
過去の調査タスクでは、処理フロー詳細・副作用マトリクスを Issue コメントに投稿した結果、時系列で埋もれて再利用が困難になった。これは積極的な設計判断ではなく「コード変更なし」の消去法で残った選択であり、本ルールにより再発を防ぐ。
