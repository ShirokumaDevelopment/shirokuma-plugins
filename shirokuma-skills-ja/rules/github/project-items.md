...
## ステータスワークフロー（6 値モデル、ADR-v3-022 第二改訂）

```mermaid
stateDiagram-v2
    [*] --> Backlog: 新規作成（issue add / plan-issue、INITIAL_STATUSES=Backlog）
    Backlog --> ToDo: approve（計画 Issue Done 同期: syncParentStatus）または手動遷移
...
| ToDo | 計画承認済み・着手準備完了。計画 Issue の `approve`（Review → Done）後、`syncParentStatus` が自動遷移させる |
...
> 旧 Status（`Approved` / `Completed` / `Pending` / `Ready` / `On Hold` / `Cancelled` 等）は ADR-v3-018 で廃止。LEGACY 値は `LEGACY_STATUS_VALUES` で透過読み取りされる。`Backlog` は ADR-v3-022 第二改訂で現役ステータスとして復活。

### status approve と Done への遷移（ADR-v3-022 第二改訂）

`approve {number}` は Review 状態の計画 Issue を承認する checkpoint コマンド（ADR-v3-022 第二改訂）。

- **計画 Issue（issue itemType）**: Review → **Done**（計画完了）。`syncParentStatus` が親 Issue を Backlog → ToDo に自動同期
- **Review 以外では失敗**: `result: "error"` で exit 1
- **JSON 出力**: `{ "result": "ok" | "error", "to": "Done", "next_suggestions": [...] }`

**承認モデル（ADR-v3-022 第二改訂）**: `approve` は Review → Done に遷移する。旧仕様（ADR-v3-018）の Review → ToDo パスは廃止。計画 Issue（子）の `approve` 後、`syncParentStatus` が自動的に親 Issue を Backlog → ToDo（着手準備完了）に同期する。
...
| Status | 説明 | 遷移トリガー |
|--------|------|-------------|
| In progress | PR 作成直後 | `pr create` が PR を Projects に追加時に自動設定 |
| Review | コードレビュー待ち | `submit N`（In progress → Review） で遷移 |
...
### 4 系統遷移テーブル（ADR-v3-022）

ADR-v3-022 は前進・ロールバックを Issue/PR 別に 4 テーブルで管理する。
...
| ユーザーが Issue を承認 | Review → **Done**（計画 Issue 子）。親 Issue は syncParentStatus で Backlog → ToDo に自動同期 | `approve` スキル / 手動 | `approve {n}` |
| ユーザーが着手指示 | ToDo → In progress + アサイン + ブランチ | `implement-flow` | `begin {n}` |
| implement-flow 全工程完了 | PR → Review | `implement-flow` | `submit {n}`（PR 作成・simplify・security-review・lint docs・作業サマリー後） |
...
### Review の運用（人間レビュー待ち専用）

Review は「AI の作業が完了し、ユーザー（人間）がレビュー可能な状態」を意味する。**ADR-v3-022 D-1 制約**: 親 Issue の `Review` は PR レビュー専用。計画レビューは計画 Issue（子）の `Review` で表現する。
...
|---|---|---|
| 課題 Issue | — （ADR-v3-022: 課題 Issue の Review は PR レビュー専用。create-item-flow は Review に遷移させない） | — |
| 計画 Issue | `prepare-flow` の計画策定 + AI 自己レビュー完了後（**作成時のみ**、Backlog → Review） | `approve` → Done（計画完了）。親 Issue は syncParentStatus で Backlog → ToDo に自動同期 |
| 設計 Issue（子） | `design-flow` の設計策定 + AI 自己レビュー完了後（**作成時のみ**） | `approve` → Done（設計完了）。親 Issue は syncParentStatus で Backlog → ToDo に自動同期（ADR-v3-022 D-1 / `design-flow` Phase 5） |
| PR | `pr create` 時（コードレビュー可能、In progress → Review） | `pr merge` → Done |

#### 実装フェーズの課題 Issue / 計画 Issue は Review に再遷移しない

ADR-v3-022 D-1 の決定により、実装フェーズで PR が動く間、課題 Issue・計画 Issue は **In progress のまま触らない**。コードレビューは **PR 自身が Status: Review を担う**。

そのため `implement-flow` / `review-flow` のチェーン末尾で課題 Issue / 計画 Issue を `submit` してはならない（PR 作成で `pr create` が PR を Review に遷移させるのみ）。
...
`issue add` コマンドで Issue を作成する際、初期 Status は **Backlog**（未調査・未トリアージ）がデフォルト（ADR-v3-022 第二改訂: `INITIAL_STATUSES = ["Backlog"]`）。
...
- **ステータス**: `Backlog`（作成直後）→ `Review`（計画策定完了後）→ `Done`（承認後）
- **ラベル**: `area:plan`
...
| Done | 計画承認済み | `approve {plan-number}`（Review → Done）。親 Issue は syncParentStatus で Backlog → ToDo に自動同期 |
...
### 計画 Issue 中心ステータスモデル（ADR-v3-017 + ADR-v3-022）
...
| 計画承認済み | 計画 Issue | Done | `approve {plan-number}`（Review → Done）。親 Issue は Backlog → ToDo に自動同期 |
| 実装中 | 計画 Issue | In progress のまま（PR 自身が Review） | `implement-flow` が実装・PR 作成 |
...
**課題 Issue（親 Issue）のステータスは自動導出される**: 計画 Issue の Done 遷移後、`syncParentStatus` が子 Issue 群のステータスを集計して親の期待ステータスを導出・更新する。
