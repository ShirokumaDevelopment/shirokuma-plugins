...
stateDiagram-v2
    ...
    Review --> Done: approve（計画/設計 Issue 子）/ pr merge（PR）
    Done --> [*]
```
...
### status approve と Done への遷移

`approve {number}` は Review 状態の Issue を承認する checkpoint コマンド。`issue_kind` で遷移先が分岐する。

- **計画 Issue / 設計 Issue（子, plan/design 分岐）**: Review → **Done**（計画/設計完了）。`syncParentStatus` が親 Issue を Backlog → ToDo に自動同期
- **課題 Issue（normal 分岐 = トリアージ承認）**: Review → **ToDo**（着手可）。親同期なし。次フローが `begin` で着手する
- **Review 以外では失敗**: `result: "error"` で exit 1
- **JSON 出力**: `{ "result": "ok" | "error", "to": "Done" | "ToDo", "next_suggestions": [...] }`

**承認モデル**: `approve` は `issue_kind` で分岐する。計画/設計 Issue（子）は Review → Done に遷移し、`syncParentStatus` が親 Issue を Backlog → ToDo（着手準備完了）に同期する。課題 Issue（トリアージ）は Review → ToDo に遷移する（親同期なし）。

> **`Review → ToDo` の位置付け**: ADR-v3-018 で廃止された「**計画承認**の Review → ToDo」とは**別概念**。本パスは**課題 Issue のトリアージ承認**（未トリアージ Backlog → トリアージ承認待ち Review → 承認済 ToDo）であり、課題トリアージ用に再導入されたものである。計画/設計 Issue の承認は引き続き Review → Done（その後 syncParentStatus が親を Backlog → ToDo に同期）。
...
| `Review` | `ToDo` | `approve`（課題 Issue トリアージ承認: normal 分岐） |
| `Review` | `Done` | `approve`（計画/設計 Issue 子）/ `pr merge`（PR） |
...
計画レビューは計画 Issue（子）の `Review` で表現し、`approve`（Review → Done）で承認する。
...
| 設計 Issue（子） | `design-flow` の設計策定 + AI 自己レビュー完了後（**作成時のみ**） | `approve` → Done（設計完了）。親 Issue は syncParentStatus で Backlog → ToDo に自動同期（`design-flow` Phase 5） |
...
> **計画 Issue 子の Review との区別**: 上表は**課題 Issue（親）**のステータスに対する共通ゲート。`prepare-flow` は加えて**計画 Issue（子）**の Review（＝再計画確認、`approve` で Review → Done）を区別して扱う。詳細は `prepare-flow` スキルのステップ 1b を参照。
