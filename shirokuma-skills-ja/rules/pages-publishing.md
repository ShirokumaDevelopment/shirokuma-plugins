# 公開資料サイト（pages/ submodule）運用ルール

`pages/` は静的サイトを配信する submodule。HTML 解説・レビュー結果・障害報告などを Issue / PR / Discussion からリンクするための公開資料置き場で、`writing-html-explainer` スキルが生成する単一ページ HTML をここに置く。
...
1. 公開資料用リポを用意する。GitHub Pages を使うなら **public** 推奨。**サイトはドメインルート配信前提**（各ページは絶対パス `/assets/...` を使う）なので、独自ドメイン（CNAME）か org pages リポ（`<org>.github.io`）を推奨する。project pages のサブパス配信（`<org>.github.io/<repo>/`）は絶対パスが壊れるため避ける
...
7. `pages.baseUrl` を確定した公開 URL に更新
...
> **必須**: ステップ 3（submodule の `main` push）。配信方式が何であれ、push しない限り公開されない。
...
- **公開サイト**なので機密情報を含めない（コード片・スクリーンショット等の公開可否を確認）
- 一度公開した URL は変えない（Issue / PR からのリンク切れ防止）
