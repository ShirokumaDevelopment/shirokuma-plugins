...
ユーザー向け設定ファイル（`.shirokuma/config.yaml` 等）はシンプルに保つ。内部実装の戦略パラメータ（`fetchStrategy`、`repoPath`、`branch`、`stripLinePattern` 等）をユーザーに書かせない。
...
- `shirokuma-flow issue` CLI を使用（直接 `gh` は禁止）
