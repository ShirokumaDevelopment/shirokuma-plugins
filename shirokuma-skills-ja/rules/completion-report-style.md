---
scope: default
category: general
priority: required
---

# 完了レポートスタイルガイド

スキルの完了レポートに「同じシステムから出力されている安心感」を実現する。統一感であって統一ではない。

## 見出し

`## {動作の名詞}完了` を標準見出しにする。

| 良い例 | 悪い例 |
|--------|--------|
| `## コミット完了` | `## 結果` |
| `## 計画完了: #42 タイトル` | `## サマリー表示` |
| `## ルール作成完了` | `## Output` |

コンテキスト付加が有用な場合は `: #{number} {title}` を追加可。

## 概要段落（推奨）

KV ペアの前に、**何をしたかを1〜2文で述べる概要段落**を置く。

```
## コミット完了

`github-writing-style` と `completion-report-style` の2ルール、および5スキルの書き込みテンプレートを更新した。

**ブランチ:** feat/2090-writing-style
**コミット:** abc1234 refactor: update writing rules
**ファイル:** 7 ファイル変更
**プッシュ:** yes
```

概要段落は**変更の意図・範囲**を端的に伝えるため、KV ペアのみのレポートより文脈が明確になる。単一ファイルの軽微な変更など、KV ペアだけで意図が自明な場合は省略してもよい。

## キーバリュー表示

`**{ラベル}:** {値}` 形式を使用。

```
**ブランチ:** feat/42-xxx
**ステータス:** Review
**場所:** .claude/rules/my-rule.md
```

## 構造の選択

| データ形状 | 構造 |
|-----------|------|
| 2項目以下のキーバリュー | `**Key:** value` 箇条書き |
| 3列以上の多列データ | テーブル |
| 順序のあるリスト | 番号付きリスト |

GitHub 書き込み（Issues, PRs, Discussions）での箇条書き vs 散文の詳細判定は `github-writing-style` ルールを参照。

## GitHub 参照

| 対象 | 形式 |
|------|------|
| Issue / PR / Discussion | `#123` |
| 外部リンク | フル URL |
| ファイルパス | バッククォート `path/to/file` |

## 次のステップ

| 呼び出し方 | 次のステップ |
|-----------|------------|
| チェーン（implement-flow 経由） | 省略（チェーンが自動実行） |
| スタンドアロン | 必ず提案 |

## スキルカテゴリ別ガイド

| カテゴリ | テンプレート要否 | 例 |
|---------|----------------|-----|
| Workflow | 必須（厳格） | commit-issue, open-pr-issue |
| Config Management | 必須（厳格） | coding-claude-config |
| Analysis | 必須（テーブル含む） | evolving-rules, prepare-flow |
| Display / Orchestrator | 不要 | showing-github, implement-flow |
| Delegator | 呼び出し元に委ねる | review-issue, issue-flow |

テンプレート不要のスキルは完了レポートセクションを持たなくてよい。

## 情報階層

完了レポートの情報は以下の優先度で配置する:

```
Level 1: 見出し（結果 + コンテキスト ID）     -- 1 秒スキャン
Level 2: 概要段落（変更の意図・範囲）          -- 3 秒スキャン
Level 3: KV ペア（重要事実）                   -- 5 秒スキャン
Level 4: テーブルまたはリスト（詳細）           -- 15 秒スキャン
Level 5: 散文説明（根拠）                      -- 30 秒+ 読み込み
```

**1 秒テスト**: 見出し + 概要段落の最初の1文だけで、結果と成果物の場所がわかること。

## フィールド分類

テンプレート内の各フィールドを以下に分類する:

| 分類 | 意味 | テンプレート表記 |
|------|------|----------------|
| Fixed | 常に存在、同一形式 | `**ラベル:** {値}` |
| Conditional | 該当時のみ表示 | `[**ラベル:** {値}]` |
| Freeform | 可変長コンテンツ | `{説明}` |

Fixed フィールドの省略・Conditional フィールドの常時表示は避ける。

## 禁止事項

- GitHub 参照に URL 直書き（`https://github.com/.../issues/42` → `#42`）
- GitHub 参照に `[#123](URL)` マークダウンリンク（`#123` で十分）
- 見出しに動詞形（`## レポートを生成する` → `## レポート生成完了`）
- セクション名のブレ（「確認表示」「サマリー表示」「レポート生成」を混用しない）
- プロセス説明（「まずファイルを読んで…」→ 結果のみ記述）
- 自己言及的な冗長表現（「正常に完了しました！」→ 事実のみ記述）
