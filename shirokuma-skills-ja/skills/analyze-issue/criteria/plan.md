...
- 計画 Issue が複数並存していれば **Critical** で「採用しない方を `issue cancel` で破棄（cancel が親からの unparent を自動実行する、#2252）、または『#X を破棄し #Y を採用』を明示宣言」を要求
...
新規ルール（`.shirokuma/rules/shirokuma-flow/*.md` 等）の TypeScript サンプルコードが実装 API と一致しているか確認する。
