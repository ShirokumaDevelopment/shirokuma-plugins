...
**HTML 化判定スキップ条件**: ロール判定後、PASS 判定で指摘 0 件かつレポート本文 80 行未満が確定している場合、ステップ 7 の構造化出力で `html_report_required: false` を返してよい（閾値の正本は `html-report-criteria.md` §2 を参照）。**ただし `requirements` ロール（`report_type: requirements-review`）は `always_html_types` 対象のため本スキップ条件の適用外。常に `html_report_required: true` を返す。**
