...
> **HTML テンプレート列**: ステップ 7「HTML 化判定」で呼び出し元オーケストレーターに返却する `template_name` 値。テンプレートとロールの対応の正本は [`.shirokuma/rules/shirokuma-flow/html-report-criteria.md`](../../../../.shirokuma/rules/shirokuma-flow/html-report-criteria.md) §3 を参照。

**HTML 化判定スキップ条件**: ロール判定後、PASS 判定で指摘 0 件かつレポート本文 80 行未満が確定している場合、ステップ 7 の構造化出力で `html_report_required: false` を返してよい（閾値の正本は `html-report-criteria.md` §2 を参照）。
...
10. **コメント投稿先**: レビュー結果は**計画 Issue (子) にコメントとして投稿**する（親 Issue ではなく `{plan-issue-number}` にコメント）。これにより計画 Issue (子) が Review Status になった際に人間レビュアーがコメントを直接参照できる（ADR-v3-022 D-1）
...
| コンテキスト | 出力先 |
|-------------|--------|
| Issue 番号指定（plan ロール） | **計画 Issue (子) にコメント**（ADR-v3-022: 計画 Issue の Review 状態でのレビュー参照を可能にするため。後方互換: 計画 Issue なしの場合は親 Issue にコメント） |
| Issue 番号指定（requirements ロール） | Issue コメント |
| Issue 番号指定（design ロール） | Issue コメント |
| Issue 番号指定（research ロール） | Issue コメント |

> 出力先ポリシーの全体像は `rules/output-destinations.md` を参照。特に「調査・設計成果物の記録先」セクション（#2251）は plan/requirements/design/research 各ロールが生成する中間成果物（処理フロー・副作用マトリクス・ギャップ分析・設計提案等）の振り分け基準を定めている — Issue コメントに偏らず、性質に応じて Discussion (Reports/Research/Knowledge) や ADR を選ぶこと。
...
判定基準・テンプレート対応・カテゴリマッピングの**正本**は [`.shirokuma/rules/shirokuma-flow/html-report-criteria.md`](../../../../.shirokuma/rules/shirokuma-flow/html-report-criteria.md) を参照する（閾値値・テンプレート名・カテゴリ名を本ファイルに直書きしないこと）。
...
`prepare-flow` から plan ロールで起動された場合、計画レビュー結果を**計画 Issue (子) にコメントとして投稿**し、以下の判定を明示する（ADR-v3-022 D-1: 計画 Issue (子) が Review Status に遷移した後、人間レビュアーが同一コンテキストでコメントを参照できるようにするため）。
