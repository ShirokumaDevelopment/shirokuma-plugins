...
| 3 | `.shirokuma/config.yaml` | ファイル存在確認 | 自動 |
| 4 | プラグインインストール | `claude plugin list` | 自動 |
| 5 | `.claude/rules/shirokuma/` デプロイ | ディレクトリ存在確認 | 自動 |
| 6 | GitHub Projects V2 | `shirokuma-flow issue list` | 自動 |
...
| config ファイルが見つからない | `shirokuma-flow init --project .` |
| プラグインが古い | `shirokuma-flow update` |
