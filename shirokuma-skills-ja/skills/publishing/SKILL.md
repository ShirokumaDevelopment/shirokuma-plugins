---
name: publishing
description: shirokuma-flow repo pairs CLIを使用して公開リリースを管理します。ステータスチェック、ドライランプレビュー、リリース実行、.shirokumaignore設定を処理します。トリガー: 「publish」「release」「公開リリース」「リリース」。
allowed-tools: Bash, Read, Grep, Glob, AskUserQuestion, TaskCreate, TaskUpdate, TaskGet, TaskList
---

# パブリッシング

`shirokuma-flow repo pairs` CLI で Private → Public リポジトリへのリリースを管理。
...
- `shirokuma-flow` CLI が PATH で利用可能
- `gh` CLI 認証済み（`gh auth status`）
- `.shirokuma/config.yaml` がプロジェクトルートに存在
- リポジトリペアが設定済み（`shirokuma-flow repo pairs list`）
...
設定は `.shirokuma/config.yaml` に保存される。
...
```bash
shirokuma-flow repo pairs list  # 設定済みの除外パターンを表示
```
