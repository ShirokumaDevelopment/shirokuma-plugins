...
> **詳細**: 7 項目の完全な定義（フォント名・カラーパレット・SVG 追従ルール等）は [`/specs/skill-writing-html-explainer/#design-output-constraints`](https://shirokuma-flow-pages.gadget.to/specs/skill-writing-html-explainer/#design-output-constraints) を参照。
...
| 恒久的な仕様書（スキル・ルール・ワークフロー・CLI） | `specs` | `pages/specs/{topic-slug}/` |
| Issue 補足の手書きトピックページ（特定 Issue に紐づく一時的な補足） | `explainers` | `pages/explainers/{topic-slug}/` |
| コードレビュー結果 | `reviews` | `pages/reviews/{topic-slug}/` |
| 設計レビュー結果 | `issues` | `pages/issues/{issue-number}/` |
...
| 項目 | 例 |
|------|-----|
| カテゴリ | `specs`（仕様書）または `explainers`（Issue 補足） |
...
|---------------|--------------|------|------------|
| `default` | `template.html` | コンセプト解説・自由構成のドキュメント | `writing-html-explainer` 直接呼び出し、`specs`・`explainers` 系全般 |
...
#### 出力ファイル名の指定（`--output-filename`）

呼び出し元は `--output-filename` パラメータで出力ファイル名を指定できる。同一 slug ディレクトリ配下にマスターページ + サブページを生成するマルチファイル出力に使う。

| パラメータ | 説明 |
|-----------|------|
| `--output-filename` | 出力ファイル名（省略時: `index.html`）。`summary.html` / `review-r{n}.html` / `fix-r{n}.html` のようなサブページ生成に使用する。指定した場合も OUTDIR（`pages/<category>/<slug>/`）は変わらない |
...
| Issue #N の設計レビュー | `design-review` | `issues` | `{N}` |
| 2026-05 のインシデント報告 | `postmortem` | `incidents` | `2026-05-outage` |
| エピック #N 計画書 | `implementation-plan` | `issues` | `{N}` |
| 恒久仕様書（スキル解説等） | `default` | `specs` | `skill-implement-flow` |
| Issue 補足（一時的な詳細解説） | `default` | `explainers` | `concept-workflow` |
...
mkdir -p ${OUTDIR}
# --output-filename が指定された場合はそのファイル名を使用（省略時は index.html）
...
`--output-filename` でサブページ（`summary.html` / `review-r{n}.html` / `fix-r{n}.html`）を生成する際、オーケストレーターは同 slug の `index.html`（マスターページ）にリンクを追記する責任を持つ。追記位置は `<!-- SUBPAGE_LINKS_START -->` と `<!-- SUBPAGE_LINKS_END -->` コメントマーカーで囲まれた領域を使用する。

マスターページが未生成の場合は先に `--output-filename index.html`（省略時と同じ）でマスターページを生成し、コメントマーカーを含むリンクリスト領域を設ける。

#### PR マスターページ（`pages/prs/{n}/index.html`）

PR 専用の軽量マスターページ。`--template default` で生成し、`<!-- SUBPAGE_LINKS_START -->` / `<!-- SUBPAGE_LINKS_END -->` マーカー間にサブページリンクを機械的に追記する構造とする。

```html
<!-- SUBPAGE_LINKS_START -->
<ul>
  <li><a href="summary.html">作業サマリー</a></li>
  <li><a href="review-r1.html">コードレビュー R1</a></li>
</ul>
<!-- SUBPAGE_LINKS_END -->
```
...
| ファイル | 更新する `?v=N` の対象 |
|---------|----------------------|
| `pages/specs/*/index.html`, `pages/explainers/*/index.html`, `pages/issues/*/index.html` 等の全ページ | `<link href="/assets/style.css?v=N">` / `<script src="/assets/theme.js?v=N">` |
...
| 論点を強調 | `.qbox`（未解決）/ `.qbox.resolved`（決定済・緑✓）。設計論点は未解決を上・決定済を下に分離 |
...
`docs/workflow-review/index.html` がリファレンス実装（submodule 移行前の試作）。submodule 整備後は `pages/specs/workflow-review/` に移動済み（#2732）。
...
- JA/EN 同期チェック観点（行数閾値・プレースホルダ数・コードブロック数）の正本は [`html-report-criteria.md` §7](../../rules/html-report-criteria.md)。
