...
- **リセット（全サブ Issue をキャンセルして再計画）**: 以下の手順で実行:
  1. 計画 Issue 以外の全サブ Issue を `shirokuma-flow issue cancel {sub-numbers}` で **Done(NOT_PLANNED)** に変更（内部で `state_reason: not_planned` の Close + Status: Done として記録、#2204。`issue cancel` は自身の親からの unparent も自動実行する（#2252）。子 Issue がある場合はその子 Issue も自動 unparent される（#2024 Phase 2-A））
...
**計画 Issue 中心モデル（ADR-v3-022、D-1）**: `prepare-flow` が管理するのは**計画 Issue (子) のライフサイクル**であり、課題 Issue（親 Issue）のステータスを直接操作しない。親 Issue のステータスは `syncParentStatus` が計画 Issue のステータスから自動導出する。
...
> **計画 Issue 中心モデル（ADR-v3-022 第二改訂）**: `prepare-flow` が管理するのは「**計画 Issue (子)** を Backlog → Review に遷移させること」のみ。課題 Issue（親 Issue）を `In progress → Review` に遷移させてはならない（ADR-v3-022 D-1: 親 Issue の Review は PR レビュー専用）。計画 Issue (子) の `approve`（`Review → Done`）による `syncParentStatus` が親 Issue を `Backlog → ToDo` に自動同期する。
