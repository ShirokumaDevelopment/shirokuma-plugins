...
autonomous-flow（マネージャー、Skill ツールで各フローを起動）
  ...
  ├─ implement-flow（Skill）    → 実装〜PR
  └─ pr merge（Bash）           → PR マージゲートを自動通過（--autopilot 時）
```
...
| 4 | Issue ループを実行する（各 Issue を全フェーズ処理） | Issue ループを実行中 | Skill: `design-flow`/`prepare-flow`/`implement-flow` + Bash |
...
Skill(skill: "implement-flow", args: "#{plan-issue-number}")
  → PR マージゲート（--autopilot 時）:
     ...
     # PR が存在する場合のみマージ（no-changes パスでは PR が存在しない）
...
| Skill | `design-flow` / `prepare-flow` / `implement-flow` の起動 |
