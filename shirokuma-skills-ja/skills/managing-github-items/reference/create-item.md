...
`issue-flow` 経由で呼ばれた場合はタイトル・種別・フィールド等の引数が渡されるため、以下の推定は不要。
...
| ラベル（area） | 会話コンテキスト | 影響範囲から `area:<領域>` 形式で推定（語彙はプロジェクトの GitHub Project 定義に従う） |
...
Issue 作成時、ユーザーの指示が「手段（何をするか）」のみで「目的（誰が・何を・なぜ）」を含まないケースを検出する。詳細な判定基準は `issue-flow` スキルを参照。
...
ラベル（影響範囲分類用 — area ラベル）:

`area:<領域>` 形式で影響するコードベース領域を分類する。具体的なラベル語彙は**各プロジェクトの GitHub Project 定義に依存する**ため固定値は持たない。`shirokuma-flow issue fields` 等で対象プロジェクトの既存 area ラベルを確認し、コンテキストから推定した影響範囲に対応するものを付与する。

## ステップ 2: 本文生成

> **CLI テンプレート**: `shirokuma-flow issue template issue --output <file>` で Issue テンプレート骨格を生成できます。AI はこのテンプレートを Read して構成を理解し、コンテキストに基づいて書き込みます。
...
| タイミング | パターン | 例 |
|-----------|---------|-----|
| 作成前 | `/tmp/shirokuma-flow/{slug}-body.md` | `/tmp/shirokuma-flow/add-format-option-body.md` |
| 作成後 | `/tmp/shirokuma-flow/{number}-body.md` 可 | `/tmp/shirokuma-flow/42-body.md` |
...
shirokuma-flow issue add /tmp/shirokuma-flow/{slug}.md
# 作成後: shirokuma-flow issue parent {number} {parent-number}
