...
### アノテーション アンチパターン（shirokuma-flow）
...
PR の成果物（Discussion / Issue）をレビューする際に適用する観点。成果物レビューサブステップ（`review-issue/SKILL.md` 参照）でのみ使用する。

### レビューチェックリスト

#### フォーマット準拠
- [ ] Discussion の場合、カテゴリに適したフォーマットになっているか（ADR なら ADR 形式、Knowledge なら Knowledge 形式）
- [ ] Issue の場合、`## 目的` / `## 概要` / `## 背景` / `## 成果物` の必須セクションが揃っているか
- [ ] タイトル形式が正しいか（ADR: `ADR-{NNN}: {タイトル}`、Knowledge: トピック名、Research: `[Research] {トピック}`）

#### YAML フロントマター混入チェック
- [ ] 本文に `---` で始まるメタデータブロックが混入していないか
- [ ] フロントマターが Markdown 本文として表示されていないか

#### クロスリファレンス整合性
- [ ] 参照している `#N` が実際に存在するか（番号の誤りがないか）
- [ ] ADR の場合、参照している他の ADR 番号が正しいか
- [ ] `Closes #N` / `Refs #N` の参照先が文脈と一致しているか

#### コードベースとの一致
- [ ] ドキュメントに記載されたコマンド・パス・API が実際の実装と一致しているか
- [ ] バージョン情報がコードベースと整合しているか（`package.json`、設定ファイル等）
- [ ] サンプルコードが動作する形式になっているか

#### 表記の統一性
- [ ] 同一ドキュメント内で同じ概念に同じ用語を使用しているか（「スキル」と「Skill」の混在等）
- [ ] 数値・単位・略称の表記が統一されているか

### アンチパターン

| パターン | 問題 |
|---------|------|
| フロントマター混入 | `---` で始まるメタデータが本文に表示される |
| 存在しない番号への参照 | `#999` など実在しない Issue/Discussion を参照 |
| コマンドパス不一致 | ドキュメント記載のパスが実際のファイル構造と異なる |
| 表記ゆれ | 同一概念に複数の表記（例: `review-worker` と `review worker`） |
| 古いバージョン情報 | 現在のコードと異なるバージョン番号 |
