...
### 6. 成果物レビュー結果（成果物レビュー対象がある場合のみ）

**PR の成果物として作成・更新された GitHub ドキュメント（Discussion / Issue）のレビュー結果**

> このセクションは「成果物レビュー対象:」がある場合のみ表示する。成果物がない場合は省略する。

| 成果物 | 種別 | 評価 | 指摘 |
|--------|------|------|------|
| #N タイトル | Discussion/Issue | OK/要修正 | 指摘内容（なければ「—」） |
...
```bash
# frontmatter に title: "[レビュー] security: apps/admin/lib/actions/" と category: Reports を設定してから実行
