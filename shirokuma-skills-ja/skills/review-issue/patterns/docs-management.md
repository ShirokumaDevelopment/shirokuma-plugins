...
- ✅ ユーティリティ作成: `packages/database/src/form-utils.ts`
- ✅ テスト追加: 19 テストケース、カバレッジ 100%
- ✅ `@repo/database` からエクスポート
- ⏳ Server Actions のリファクタ（次の推奨ステップ）  ← 完了しているか確認

## Next Steps  ← まだ有効か確認

1. `apps/admin/lib/actions/*.ts` ファイルのリファクタ
```
...
```markdown
<!-- 例: これはデッドリンクのアンチパターン -->
詳細は [実装詳細](./example-feature-doc.md) を参照
```
...
# 古いバージョン参照を検索
grep -r "16\.0\.[0-9]" docs/  # Next.js バージョンを確認
```
