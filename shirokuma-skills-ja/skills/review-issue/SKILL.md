...
レポート保存後、本スキル自身は HTML 生成を行わず、呼び出し元オーケストレーター（`review-flow` 等）に**判定情報**を構造化データで返却する。判定基準・返却フィールド・テンプレート選択の詳細は [reference/html-report-decision.md](reference/html-report-decision.md) を参照（正本: [`.shirokuma/rules/shirokuma-flow/html-report-criteria.md`](../../../../.shirokuma/rules/shirokuma-flow/html-report-criteria.md)）。
...
Skill ツール経由でメインコンテキスト実行 → `.claude/rules/` にアクセス可能。進捗報告例は `reference/progress-report-examples.md` 参照。スタンドアロン起動時は完了後に `/commit-issue` を提案する。分析が不完全な場合はカバレッジ不足箇所を特定し追加パターンを読み込む。
...
- **コメントファースト**: このスキルはコメント投稿のみ行い本文更新は行わない（`item-maintenance.md` 参照）
