...
**`implement-flow` チェーンからサブエージェントとして呼ばれた場合**: このステップ（ステップ 7）全体をスキップする。チェーンの次ステップ（PR 作成）は呼び出し元のマネージャー（メイン AI）が制御する。ここで PR チェーンを起動したり次のステップを提案すると、チェーンの制御が分断される。
