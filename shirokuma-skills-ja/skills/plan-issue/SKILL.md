...
計画作成完了のため、`submit` で Backlog → Review に遷移する（計画 Issue は作成直後は Backlog）:
...
- 計画 Issue は `Backlog` で作成し、`submit` で `Backlog → Review` に遷移する
...
| エピック Issue（サブ Issue あり） | エピック計画テンプレートを使用し、integration ブランチ名とサブ Issue 構成を含める（計画 Issue 自体はサブ Issue カウントから除外） |
