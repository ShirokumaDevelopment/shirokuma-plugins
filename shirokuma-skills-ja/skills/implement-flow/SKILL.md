...
| 4 | コードを後処理する（簡略化・セキュリティレビュー・lint docs・改善コミット） | コードを後処理中 | `finalize-changes`（Skill ツール） |
| 5 | 作業サマリーを投稿する | 作業サマリーを投稿中 | マネージャー直接: `issue comment` |
| 6 | 次フロー（`/review-flow`）を案内提示する（**Issue 本体・PR を Review に遷移させない。PR は Backlog のまま**）| 次フローを案内中 | マネージャー直接 |

Dependencies: step 2 blockedBy 1, step 3 blockedBy 2, step 4 blockedBy 3, step 5 blockedBy 4, step 6 blockedBy 5.

> **PR の初期 Status（#2802）**: ステップ 3 で起動された `pr create` は PR 自身を **Status: Backlog** で作成する。

> **implement-flow はコードレビューを自動起動しない（#2818）**: 実装チェーンは PR 作成後に AI レビュー（`review-flow`）を自動起動しない。コードレビューは「新しい会話のエントリーポイント」であり、ユーザー制御ポイント（UCP）を経て起動されるべき作業である。チェーン末尾（ステップ 6）では `/review-flow #{PR#}` を**案内提示するにとどめ**、起動はユーザーに委ねる。
>
> **PR は Backlog のままチェーンを終える**: `Backlog → Review` 遷移は、ユーザーが `/review-flow` を実行し AI レビューが PASS した時点で `review-flow` が行う。implement-flow チェーンは PR を Review に遷移させない。課題 Issue / 計画 Issue も Review に遷移させない（1 エンティティ 1 Review 原則）。
>
> **finalize-changes は維持（ステップ 4）**: 後処理（`/simplify` + `reviewing-security` + `lint docs` + 改善コミット）はコードレビューとは別の後処理であり、ユーザー制御対象外のため PR 作成後に自動実行する。

> **変更なし分岐**: `coding-worker` が `changes_made: false` で完了した場合、ステップ 2〜4（コミット・PR・finalize-changes）をスキップし、ステップ 5（変更なし用作業サマリー）→ ステップ 6（次フロー案内 / ステータス判定）に進む。詳細は [reference/chain-execution.md](reference/chain-execution.md) の「変更なしパス」および [reference/chain-end-steps.md](reference/chain-end-steps.md) の「変更なしパス」参照。
...
> **`pr create` の初期 Status（#2802 / #2818）**: `pr create` は PR を **Backlog** で作成する（旧: Review への自動遷移）。PR は Backlog で作成され、**ユーザーが `/review-flow` を実行**し AI レビューが PASS した時点で `review-flow` が PR を `Backlog → Review` に遷移させる。implement-flow チェーンは PR を Review に遷移させない。計画 Issue・課題 Issue も Review に遷移させない（1 エンティティ 1 Review 原則）。
...
| 作業タイプ | チェーン |
|-----------|---------|
| コーディング全般 | Work → Commit → PR(Backlog) → finalize-changes → Work Summary + PR ページ生成 → /review-flow 案内提示 |
| 調査 | Research → Discussion |
| レビュー | Review → レポート投稿 → 完了（コミット/PR チェーンなし） |

> **コーディングチェーンの末尾（#2818）**: `PR(Backlog)` は `pr create` が PR を Backlog で作成することを表す。PR 作成後は `finalize-changes`（後処理）を実行し、Work Summary + PR ページ生成後に `/review-flow #{PR#}` を**案内提示してチェーンを終える**。implement-flow はコードレビューを自動起動しない — レビューはユーザーが `/review-flow` で明示起動する。PR は Backlog のままチェーンを終え、`Backlog → Review` 遷移はユーザーの `/review-flow` 実行時に `review-flow` が AI レビュー PASS 後に行う。
