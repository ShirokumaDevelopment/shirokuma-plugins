...
| 3 | プルリクエストを作成する（PR は Backlog で作成） | プルリクエストを作成中 | `open-pr-issue` (subagent) |
| 4 | AI レビューを実行する（PASS で PR を Review に遷移） | AI レビュー中 | `review-flow`（Skill ツール） |
| 5 | コードを後処理する（簡略化・セキュリティレビュー・改善コミット） | コードを後処理中 | `finalize-changes`（Skill ツール） |
| 6 | 作業サマリーを投稿する | 作業サマリーを投稿中 | マネージャー直接: `issue comment` |
| 7 | PR が Review に遷移済みであることを検証する（**Issue 本体を Review に遷移させない**）| PR ステータスを検証中 | マネージャー直接: `status get <PR>` |
...
> **ステップ 4 の位置付け（AI レビューゲート）**: ステップ 3 で起動された `pr create` は PR 自身を **Status: Backlog** で作成する（#2802）。本ステップは `review-flow` を Skill ツールで起動して AI レビューを実行し、**PASS した場合のみ** `review-flow` が PR を `Backlog → Review` に遷移させる（コードレビュー完了の明示シグナル）。`review-flow` は Skill ツールで起動するオーケストレーターであり、メインコンテキスト内で完了する。
>
> **FAIL 時はチェーン停止**: AI レビューが FAIL（または未解決スレッドあり）の場合、PR は `Backlog` のまま保持され、`review-flow` がスレッド対応フローに入る。この場合はチェーンを停止し、ユーザーに制御を返す。修正→再レビューが PASS した時点で初めて PR が Review に上がる。

> **ステップ 7 の位置付け（PR 状態の検証のみ）**: ステップ 4 の `review-flow` が AI レビュー PASS 後に PR を **Status: Review** に遷移済みであることを `status get <PR>` で確認するのみ。課題 Issue / 計画 Issue を `submit` で Review に遷移させない（`project-items.md` の Review セクション「DO NOT」リスト準拠 — 1 エンティティ 1 Review 原則）。
>
> 実装フェーズでは PR 自身が `Status: Review` を担い、課題 Issue・計画 Issue は In progress のまま。`pr merge` 時に `Status: Done` に直接遷移する。
>
> **PR が Review でない場合**: AI レビューが FAIL のままチェーンが継続したケースは想定しない（FAIL 時はステップ 4 でチェーン停止）。検証で Review でなかった場合は AI レビュー未完了を意味するため、`review-flow` の状態を確認する。**課題 Issue / 計画 Issue を submit してはならない**。

> **変更なし分岐**: `coding-worker` が `changes_made: false` で完了した場合、ステップ 2〜5（コミット・PR・review-flow・finalize-changes）をスキップし、ステップ 6（変更なし用作業サマリー）→ ステップ 7（ステータス判定）に進む。詳細は [reference/chain-execution.md](reference/chain-execution.md) の「変更なしパス」および [reference/chain-end-steps.md](reference/chain-end-steps.md) の「変更なしパス」参照。
...
> **`pr create` の初期 Status（#2802）**: `pr create` は PR を **Backlog** で作成する（旧: Review への自動遷移）。コードレビュー完了の明示は `review-flow` の AI レビュー PASS 時に PR を `Backlog → Review` に遷移させることで表現する。計画 Issue・課題 Issue は Review に遷移させない（1 エンティティ 1 Review 原則）。
...
| 作業タイプ | チェーン |
|-----------|---------|
| コーディング全般 | Work → Commit → PR(Backlog) → review-flow → [Review] → finalize-changes → Work Summary + PR ページ生成 → Status Update |
| 調査 | Research → Discussion |
| レビュー | Review → レポート投稿 → 完了（コミット/PR チェーンなし） |

> **コーディングチェーンの AI レビューゲート（#2802）**: `PR(Backlog)` は `pr create` が PR を Backlog で作成することを表す。`review-flow` の AI レビューが PASS すると `[Review]`（PR を `Backlog → Review` に遷移）に進む。AI レビューが FAIL または未解決スレッドありの場合はチェーンを停止し、ユーザーに制御を返す（PR は Backlog のまま）。
...
| `review-flow` | Skill ツール | AI レビューゲートのオーケストレーター。PASS で PR を `Backlog → Review` に遷移。**Agent ツールで起動しない**（メインコンテキストで完了するオーケストレーター） |
