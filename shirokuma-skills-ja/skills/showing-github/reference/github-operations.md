...
shirokuma-flow issue update {number} --labels "area:cli,area:plugin"               # ラベル更新
...
shirokuma-flow approve {number}                                                     # Review → Done（計画 Issue の場合は syncParentStatus で親が Backlog → ToDo に同期）（checkpoint、推奨）ADR-v3-022
...
GitHub Projects V2 の Status は 6 値（ADR-v3-022）:
...
stateDiagram-v2
  ...
  InProgress --> Review: pr create（PR_FORWARD: PR のみ、ADR-v3-022 D-1）
  Review --> Done: approve（計画 Issue 子）
  Review --> Done: pr merge（PR）
  InProgress --> Blocked: block
  Blocked --> InProgress: resume
  InProgress --> Done: pr merge（課題 Issue 子: ADR-v3-022 D-1）
...
| Review | レビュー待ち（計画レビュー / コードレビュー）。ADR-v3-022 D-1 制約: 親 Issue の Review は PR レビュー専用（計画 Issue 子は別エンティティ） | `submit <N>` |
