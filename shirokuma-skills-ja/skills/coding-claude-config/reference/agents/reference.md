...
エージェントは独立したサブプロセスで実行され、**メインコンテキストのルールは自動伝搬しない**。

### 伝搬メカニズム

| メカニズム | 方法 | ユースケース |
|-----------|------|------------|
| `skills:` フロントマター | スキルを注入（スキルの reference も利用可能になる） | 特定ワークフローをエージェントに委任 |
| 初期化セクションで Read 指示 | AGENT.md 内で「最初に X を Read ツールで読む」と指示 | 特定ルールファイルの内容が必要な場合 |
| プロンプト内に直接記述 | ルールの要約をシステムプロンプトに含める | 短いルール、変更頻度が低い場合 |
| `context: fork`（Skill） | Skill ツールの `context: fork` でメインコンテキストをサブエージェントにフォーク | サブエージェントにメインコンテキスト全体（ルール + 会話）が必要な場合 |

> **注意:** スキルが `context: fork` を使用する場合、サブエージェントはメインコンテキスト（読み込み済みルールを含む）を継承する。フォークされたエージェントでは AGENT.md 内の明示的 Read 指示は不要。上記メカニズムは Agent ツール（独立サブプロセス）で起動されるエージェントにのみ適用される。

### アンチパターン

| パターン | 問題 |
|---------|------|
| AGENT.md に「branch-workflow ルールに従う」とだけ書く | サブエージェントにはルールが伝搬しないため無視される |
| ルールファイルのパスを書いて「参照」と記述 | 自動展開されない — Read ツールで明示的に読む指示が必要 |

### 推奨パターン

初期化セクションで Read を指示する場合:

```markdown
## 初期化

1. 以下のファイルを Read ツールで読み込む:
   - `.claude/rules/shirokuma/github/branch-workflow.md`
2. 読み込んだルールに従って作業する
```
