...
- **設計成果物の出力先**: `shirokuma-skills` の `rules/output-destinations.md` の「調査・設計成果物の記録先」に従う。アーキテクチャ提案や Server Actions / API Routes の設計判断は **Discussion (Knowledge)** または ADR、画面遷移マトリクスや副作用マトリクスは **Discussion (Reports)** が既定
