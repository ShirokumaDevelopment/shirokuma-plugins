...
Code templates for the coding-nextjs skill.
...
See [server-action.md](./server-action.md) for the **shirokuma-flow lint code compliant** template.
...
```bash
# Check compliance with shirokuma-flow
node shirokuma-flow/dist/index.js lint code -p path/to/project -v
```
